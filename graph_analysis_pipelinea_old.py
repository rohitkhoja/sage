...
class GraphAnalysisPipeline:
    ...
    def _analyze_doc_doc_outliers(self, df: pd.DataFrame, outlier_dir: Path):
        ...
        threshold_1 = doc_doc_df[var1].quantile(0.95)
        threshold_2 = doc_doc_df[var2].quantile(0.95)
        
        logger.info(f"Doc-Doc thresholds: {var1}={threshold_1:.3f}, {var2}={threshold_2:.3f}")
        
        # Case 1: content_similarity high, topic_similarity low
        case1_mask = (doc_doc_df[var2] > threshold_2) & (doc_doc_df[var1] < threshold_1)
        case1_outliers = doc_doc_df[case1_mask].copy()
        
        # Case 2: topic_similarity high, content_similarity low  
        case2_mask = (doc_doc_df[var1] > threshold_1) & (doc_doc_df[var2] < threshold_2)
        case2_outliers = doc_doc_df[case2_mask].copy()
    ...
    def _analyze_doc_table_outliers(self, df: pd.DataFrame, outlier_dir: Path):
        ...
        for pair_idx, (var1, var2) in enumerate(variable_pairs, 1):
            logger.info(f"Analyzing doc-table pair {pair_idx}: {var1} vs {var2}")
            
            # Calculate 80th percentile thresholds
            threshold_1 = doc_table_df[var1].quantile(0.95)
            threshold_2 = doc_table_df[var2].quantile(0.95)
            
            # Case 1: var2 high, var1 low
            case1_mask = (doc_table_df[var2] > threshold_2) & (doc_table_df[var1] < threshold_1)
            case1_outliers = doc_table_df[case1_mask].copy()
            
            # Case 2: var1 high, var2 low
            case2_mask = (doc_table_df[var1] > threshold_1) & (doc_table_df[var2] < threshold_2)
            case2_outliers = doc_table_df[case2_mask].copy()
